...
$(function(){//자동으로 실행되는 코드
	...
	$("#btnReply").click(function(){
		...
		var param={ "replytext" : replytext, "bno" : bno};//json 형식
	...
	$("#btnUpdate").click(function(){
		...
		$("#uploadedList .file").each(function(i){
			srt+="<input type='hidden' name='files["+i+"]' value='"+$(this).val()+"'>";
			});
	$("#form1").append(srt);
	...
	//첨부파일 삭제
	//id가 uploadedList인 태그의 class 가 file_del인 태그 출력
	$("#uploadedList").on("click",".file_del",function(e){
		var that=$(this);//클릭한 태그
		$.ajax({
			type:"post",
			url: "${path}/upload/deleteFile",
			//data: {변수명 : 값}=> 현제태그의 data-src의 값
...
action="${path}/board/insert.do">
	<div>제목 <input name="title" id="title" size="80" value="${dto.title}"
