...
public class NewsServlet extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action == null) {
			...
			if(list != null && list.size() != 0) {
				...
				linkStr="&action=listwriter&wirter="+writer;
			}
		} else if (action.equals("search")) {
			String key = request.getParameter("key");
			String searchType = request.getParameter("serchType");
		...
		request.getRequestDispatcher("/view/newsView.jsp").forward(request, response);
	}
	protected void doPost(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
				...
				Stirng param = request.getParameter("pgNum");
				int pgNum = param == null ? 1 : Integer.parseInt(param);
				...
				vo.setContent(tequest.getParameter("content"));
